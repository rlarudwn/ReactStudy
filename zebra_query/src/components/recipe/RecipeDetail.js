...
function Review(props) {
  ...
  let arr = []
  for (let i = 1; i <= 5; i++) {
    arr.push(<i onClick={() => props.rating(i)} className={"fa-solid fa-star"} style={{ color: i <= props.rate ? 'goldenrod' : '' , cursor:'pointer'}}></i>)
  }
  let makeRate=(rate)=>{
    let arrs=[]
    for(let i=1;i<=5;i++){
    ...
    <div className="product__details__tab__desc">
      ...
      <div className="row">
        <table className="table">
                  ...
                  <div class="product__details__rating" style={{ textAlign: 'left' }}>
                    ...
                    <table style={{ width: '100%', height: '100px' }}>
                      <tbody>
                        <tr>
                          <td width={'90%'}>
                            <textarea style={{ width: '100%', resize: 'none', height: '100px' }} ref={msgRef} onChange={(e) => props.msging(e.target.value)}></textarea>
                          </td>
                          <td width={'10%'}>
                            <button type="button" className="btn primary-btn" style={{ width: '100%', height: '100px' }} onClick={() => { props.reviewInsert() }}>작성</button>
                          </td>
                        </tr>
                      </tbody>
                    </table>
                  </div>
                </div>) : (<div>이미 리뷰를 작성했습니다</div>)) : (<div>
                  로그인 후 작성 가능합니다
                </div>)
              }
            </td>
          ...
          {
            props.list && props.list.map((review) => {
              return (
                <tr>
                  <td>
                    <div class="review-list">
                      <div>
                        <span class="nick-rate">{review.nickname}</span>
                        {
                          props.id===review.id?(<input type="button" class="xBtn" value={"X"} onClick={()=>props.reviewDelete(review.rno)}/>):(<></>)
                        }
                        <br />
                        {makeRate(review.rating)}
                        <br />
                        <span class="date">&nbsp;{review.regdate}</span>
                        <div class="r-content"><pre>{review.content}</pre></div>
                      </div>
                    </div>
                  </td>
                </tr>
            ...
            <div className="col-lg-6 col-md-6">
              <div className="product__details__text">
                ...
                <div className="product__details__rating">
                  ...
                  <span>({data.data.detail.score})</span>
